...
/**
  ...
  * @retval None
  */
extern unsigned int textshown_pos0;
extern unsigned int textshown_pos1;
extern unsigned char textbuf[];
extern unsigned int textlen;
extern void OLED_ShowString(unsigned char,unsigned char, unsigned char*,unsigned char);	
extern void delay_ms(unsigned int);
extern unsigned char IsButtonB();
INTERRUPT_HANDLER(EXTI_PORTD_IRQHandler, 6)
{
  ...
  */
	...
	disableInterrupts();
	delay_ms(100);
	temp0=textshown_pos0;
	temp1=textshown_pos1;
	//EXTI_SetExtIntSensitivity(EXTI_PORT_GPIOD,EXTI_SENSITIVITY_RISE_ONLY);	
	while(textbuf[textshown_pos0++]!=0 && textshown_pos0<textlen+1);
	while(textbuf[textshown_pos1++]!=0 && textshown_pos1<textlen+1);
	textshown_pos0+=2;textshown_pos1+=2;
	if(textshown_pos0 > textlen+1)textshown_pos0=1;
	if(textshown_pos1 > textlen+1)textshown_pos1=1;
	OLED_Clear(); 
	OLED_ShowString(0,0,(unsigned char*)(textbuf+temp0),16);
	OLED_ShowString(0,2,(unsigned char*)(textbuf+temp1),16);
	while(IsButtonB()==0);
	delay_ms(100);
	enableInterrupts();
}
