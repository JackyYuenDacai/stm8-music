...
)
{
	...
	case FS_FAT32 :
		...
		return LD_DWORD(buf) & (DWORD)0x0FFFFFFF;
#endif
	}
...
)
{
	...
	clst -= 2;
	if (clst >= ((DWORD)fs->n_fatent - 2)) return 0;		/* Invalid cluster# */
	return (DWORD)clst * (DWORD)fs->csize + (DWORD)fs->database;
}


static
CLUST get_clust (
	volatile BYTE* dir		/* Pointer to directory entry */
)
{
	volatile FATFS *fs = FatFs;
	volatile CLUST clst = 0;


	if ((_FS_FAT32 && fs->fs_type == FS_FAT32)||_FS_32ONLY ) {
...
static
FRESULT dir_rewind (
	volatile DIR *dj			/* Pointer to directory object */
)
{
	CLUST volatile clst;
	FATFS  volatile *fs = FatFs;
	...
	if (clst == 1 || clst >= fs->n_fatent)	/* Check start cluster range */
		return FR_DISK_ERR;
	if (!clst && (fs->fs_type == FS_FAT32||_FS_32ONLY  )&&_FS_FAT32  )	/* Replace cluster# 0 with root cluster# if in FAT32 */
		clst = (CLUST)fs->dirbase;
	dj->clust = clst;						/* Current cluster */
	
	dj->sect = (clst!=0) ? clust2sect(clst):fs->dirbase;	/* Current sector */
...
FRESULT dir_find (
	...
	volatile BYTE *dir		/* 32-byte working buffer */
)
{
	FRESULT res;
	BYTE c;
...
)
{
	...
	for (;;) {
		...
		if (c >= 0x80)					/* To upper extended char (SBCS) */
			c = cvt[c - 0x80];
#endif
		if (i < ni - 1 && IsDBCS1(c)) {	/* DBC 1st byte? */
...
)
{
	...
	if (!disk_readp(buf, sect, BS_FilSysType, 2)&& LD_WORD(buf) == 0x4146&& !_FS_32ONLY   )	/* Check FAT12/16 */
		return 0;
	if (!disk_readp(buf, sect, BS_FilSysType32, 2)&& LD_WORD(buf)== 0x4146&& _FS_FAT32   )	/* Check FAT32 */
		return 0;
	return 1;
}
...
)
{
	...
	volatile DWORD bsect, fsize, tsect, mclst;
...
)
{
	...
	*br = 0;
	...
	while (btr)	{									/* Repeat until all data transferred */
		if (((UINT)fs->fptr & (UINT)511) == 0) {				/* On the sector boundary? */
			cs = (BYTE)(((UINT)((UINT)fs->fptr>>((UINT)9)) & (UINT)(fs->csize - 1))&0xFF);	/* Sector offset in the cluster */
		...
		rcnt = (UINT)((UINT)512 - (UINT)((UINT)fs->fptr & (UINT)511));			/* Get partial sector data from sector buffer */
		if (rcnt > btr) rcnt = btr;
		dr = disk_readp(buff ==0? 0 : rbuff, fs->dsect, (UINT)((UINT)fs->fptr & (UINT)511), rcnt);
